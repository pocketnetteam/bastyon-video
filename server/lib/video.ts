...
function buildLocalVideoFromReq (
  videoInfo: VideoCreate,
  channelId: number
): FilteredModelAttributes<VideoModel> {
  return {
    name: videoInfo.name,
    remote: false,
    ...
    commentsEnabled:
      videoInfo.commentsEnabled ?? CONFIG.DEFAULTS.PUBLISH.COMMENTS_ENABLED,
    downloadEnabled:
      videoInfo.downloadEnabled ?? CONFIG.DEFAULTS.PUBLISH.DOWNLOAD_ENABLED,
...
async function setVideoTags (options: {
  ...
  const tagInstances = await TagModel.findOrCreateTags(
    internalTags,
    transaction
  )
...
async function addTranscodingJob (
  payload: VideoTranscodingPayload,
  options: CreateJobOptions = {}
) {
  await VideoJobInfoModel.increaseOrCreate(
    payload.videoUUID,
    "pendingTranscode"
  )

  return JobQueue.Instance.createJobWithPromise(
    { type: "video-transcoding", payload: payload },
    options
  )
}
...
async function getTranscodingJobPriority (user: MUserId) {
  ...
  const videoUploadedByUser = await VideoModel.countVideosUploadedByUserSince(
    user.id,
    lastWeek
  )
