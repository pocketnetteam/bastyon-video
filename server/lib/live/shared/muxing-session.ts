...
class MuxingSession extends EventEmitter {
  ...
  private readonly isAbleToUploadVideoWithCache = memoizee(
    (userId: number) => {
  ...
  private readonly hasClientSocketInBadHealthWithCache = memoizee(
    (sessionId: string) => {
  ...
  async runMuxing () {
    ...
    logger.info(
      "Running live muxing/transcoding for %s.",
      this.videoUUID,
      this.lTags()
    )

    await this.watchTSFiles()
    ...
    this.ffmpegCommand.on("start", (cmdline) => {
      ...
      logger.debug("Running ffmpeg command for live", {
        ffmpegShellCommand,
        ...this.lTags()
      })
    })
  ...
  private onFFmpegError (options: {
    ...
    logger.error("Live transcoding error.", {
      err,
      stdout,
      stderr,
      ffmpegShellCommand,
  ...
  private onFFmpegEnded () {
    logger.info(
      "RTMP transmuxing for video %s ended. Scheduling cleanup",
      this.inputUrl,
      this.lTags()
    )
  ...
  private watchMasterFile () {
    this.masterWatcher = watch(
      this.outDirectory + "/" + this.streamingPlaylist.playlistFilename
    )
    ...
    this.masterWatcher.on("add", () => {
      ...
      this.masterWatcher
        ...
        .catch((err) =>
          logger.error(
            "Cannot close master watcher of %s.",
            this.outDirectory,
            { err, ...this.lTags() }
          )
        )
  ...
  private async watchTSFiles () {
    ...
    const existingFiles = await readdir(this.outDirectory)

    await Promise.all(
      existingFiles.map((fileName) =>
        fileName.includes(".ts")
          ? unlink(join(this.outDirectory, fileName))
          : Promise.resolve()
      )
    )
    ...
    const addHandler = async (segmentPath: string) => {
      ...
      const segmentsToProcess =
        this.segmentsToProcessPerPlaylist[playlistId] || []
    ...
    const deleteHandler = (segmentPath: string) =>
      LiveSegmentShaStore.Instance.removeSegmentSha(
        this.videoUUID,
        segmentPath
      )
  ...
  private async isQuotaExceeded (segmentPath: string) {
    ...
    try {
      ...
      LiveQuotaStore.Instance.addQuotaTo(
        this.user.id,
        this.videoLive.id,
        segmentStat.size
      )
      ...
      logger.error(
        "Cannot stat %s or check quota of %d.",
        segmentPath,
        this.user.id,
        { err, ...this.lTags() }
      )
  ...
  private processSegments (segmentPaths: string[]) {
    mapSeries(segmentPaths, async (previousSegment) => {
      ...
      await LiveSegmentShaStore.Instance.addSegmentSha(
        this.videoUUID,
        previousSegment
      )
  ...
  private hasClientSocketInBadHealth (sessionId: string) {
    ...
    if (!rtmpSession) {
      logger.warn(
        "Cannot get session %s to check players socket health.",
        sessionId,
        this.lTags()
      )
      return
    ...
    for (const playerSessionId of rtmpSession.players) {
      ...
      if (!playerSession) {
        logger.error(
          "Cannot get player session %s to check socket health.",
          playerSession,
          this.lTags()
        )
        continue
      ...
      if (
        playerSession.socket.writableLength > VIDEO_LIVE.MAX_SOCKET_WAITING_DATA
      ) {
        return true
      }
    }
  ...
  private async addSegmentToReplay (segmentPath: string) {
    ...
    try {
      ...
      logger.error("Cannot copy segment %s to replay directory.", segmentPath, {
        err,
        ...this.lTags()
      })
    }
