...
let VideoFileModel = VideoFileModel_1 = class VideoFileModel extends sequelize_typescript_1.Model {
    ...
    static doesOwnedTorrentFileExist(filename) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            return shared_1.doesExist(query, { filename });
        });
    }
    ...
    getObjectStorageUrl() {
        if (this.isHLS()) {
            return object_storage_1.getHLSPublicFileUrl(this.fileUrl);
        }
        return object_storage_1.getWebTorrentPublicFileUrl(this.fileUrl);
    }
    getFileUrl(video) {
        if (this.storage === 1) {
    ...
    getFileDownloadUrl(video) {
        const path = this.isHLS()
            ? path_1.join(constants_1.STATIC_DOWNLOAD_PATHS.HLS_VIDEOS, `${video.uuid}-${this.resolution}-fragmented${this.extname}`)
            : path_1.join(constants_1.STATIC_DOWNLOAD_PATHS.VIDEOS, `${video.uuid}-${this.resolution}${this.extname}`);
        if (video.isOwned())
    ...
    removeTorrent() {
        if (!this.torrentFilename)
            return null;
        const torrentPath = paths_1.getFSTorrentFilePath(this);
...
VideoFileModel.doesInfohashExistCached = memoizee_1.default(VideoFileModel_1.doesInfohashExist, {
    promise: true,
