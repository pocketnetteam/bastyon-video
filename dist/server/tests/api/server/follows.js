    ...
    describe('Data propagation after follow', function () {
        ...
        it('Should have 2 followings on server 1', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                const body2 = yield servers[0].follows.getFollowings({ start: 1, count: 1, sort: 'createdAt' });
        ...
        it('Should search/filter followings on server 1', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                const count = 1;
                {
                    ...
                    {
                        const body = yield servers[0].follows.getFollowings({ start, count, sort, search });
                        expect(body.total).to.equal(1);
                        const follows = body.data;
                        expect(follows).to.have.lengthOf(1);
                    ...
                    {
                        const body = yield servers[0].follows.getFollowings({ start, count, sort, search, state: 'accepted' });
                        expect(body.total).to.equal(1);
                        expect(body.data).to.have.lengthOf(1);
                    }
                    {
                        const body = yield servers[0].follows.getFollowings({ start, count, sort, search, state: 'accepted', actorType: 'Person' });
                        expect(body.total).to.equal(1);
                        expect(body.data).to.have.lengthOf(1);
                    }
                    {
                        const body = yield servers[0].follows.getFollowings({
                            start,
                            count,
                            sort,
                            search,
                    ...
                    {
                        const body = yield servers[0].follows.getFollowings({ start, count, sort, search, state: 'pending' });
                        expect(body.total).to.equal(0);
                        expect(body.data).to.have.lengthOf(0);
                    }
                }
                {
                    const body = yield servers[0].follows.getFollowings({ start, count, sort, search: 'root' });
                    expect(body.total).to.equal(1);
                    expect(body.data).to.have.lengthOf(1);
                }
                {
                    const body = yield servers[0].follows.getFollowings({ start, count, sort, search: 'bla' });
                    expect(body.total).to.equal(0);
                    expect(body.data).to.have.lengthOf(0);
                }
            });
        ...
        it('Should search/filter followers on server 2', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                const sort = 'createdAt';
                {
                    ...
                    {
                        const body = yield servers[2].follows.getFollowers({ start, count, sort, search });
                        expect(body.total).to.equal(1);
                        const follows = body.data;
                        expect(follows).to.have.lengthOf(1);
                    ...
                    {
                        const body = yield servers[2].follows.getFollowers({ start, count, sort, search, state: 'accepted' });
                        expect(body.total).to.equal(1);
                        expect(body.data).to.have.lengthOf(1);
                    }
                    {
                        const body = yield servers[2].follows.getFollowers({ start, count, sort, search, state: 'accepted', actorType: 'Person' });
                        expect(body.total).to.equal(0);
                        expect(body.data).to.have.lengthOf(0);
                    }
                    {
                        const body = yield servers[2].follows.getFollowers({
                            start,
                            count,
                            sort,
                            search,
                    ...
                    {
                        const body = yield servers[2].follows.getFollowers({ start, count, sort, search, state: 'pending' });
                        expect(body.total).to.equal(0);
                        expect(body.data).to.have.lengthOf(0);
                    }
                }
                {
                    const body = yield servers[2].follows.getFollowers({ start, count, sort, search: 'bla' });
                    expect(body.total).to.equal(0);
                    const follows = body.data;
                    expect(follows).to.have.lengthOf(0);
                }
        ...
        it('Should upload a video on server 2 and 3 and propagate only the video of server 2', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                {
                    const { total, data } = yield servers[2].videos.list();
                    expect(total).to.equal(1);
    ...
    describe('Should propagate data on a new server follow', function () {
        ...
        before(function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                yield servers[2].videos.upload({ attributes: { name: 'server3-2' } });
                yield servers[2].videos.upload({ attributes: { name: 'server3-3' } });
                video4 = yield servers[2].videos.upload({ attributes: video4Attributes });
                yield servers[2].videos.upload({ attributes: { name: 'server3-5' } });
                yield servers[2].videos.upload({ attributes: { name: 'server3-6' } });
                {
                    const userAccessToken = yield servers[2].users.generateUserAndToken('captain');
                    yield servers[2].videos.rate({ id: video4.id, rating: 'like' });
                    yield servers[2].videos.rate({ token: userAccessToken, id: video4.id, rating: 'dislike' });
                }
                {
                    yield servers[2].comments.createThread({ videoId: video4.id, text: 'my super first comment' });
                    yield servers[2].comments.addReplyToLastThread({ text: 'my super answer to thread 1' });
                    yield servers[2].comments.addReplyToLastReply({ text: 'my super answer to answer of thread 1' });
                    yield servers[2].comments.addReplyToLastThread({ text: 'my second answer to thread 1' });
                }
                {
                    const { id: threadId } = yield servers[2].comments.createThread({ videoId: video4.id, text: 'will be deleted' });
                    yield servers[2].comments.addReplyToLastThread({ text: 'answer to deleted' });
                    const { id: replyId } = yield servers[2].comments.addReplyToLastThread({ text: 'will also be deleted' });
                    yield servers[2].comments.addReplyToLastReply({ text: 'my second answer to deleted' });
                    yield servers[2].comments.delete({ videoId: video4.id, commentId: threadId });
                    yield servers[2].comments.delete({ videoId: video4.id, commentId: replyId });
                }
                yield servers[2].captions.add({
                    language: 'ar',
                    videoId: video4.id,
