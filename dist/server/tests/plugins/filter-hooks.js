...
describe('Test plugin filter hooks', function () {
    ...
    describe('Download hooks', function () {
        ...
        it('Should run filter:api.download.torrent.allowed.result', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                const res = yield extra_utils_1.makeRawRequest(downloadVideos[0].files[0].torrentDownloadUrl, 403);
                expect(res.body.error).to.equal('Liu Bei');
                yield extra_utils_1.makeRawRequest(downloadVideos[1].files[0].torrentDownloadUrl, 200);
                yield extra_utils_1.makeRawRequest(downloadVideos[2].files[0].torrentDownloadUrl, 200);
            });
        });
        it('Should run filter:api.download.video.allowed.result', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                {
                    const res = yield extra_utils_1.makeRawRequest(downloadVideos[1].files[0].fileDownloadUrl, 403);
                    expect(res.body.error).to.equal('Cao Cao');
                    yield extra_utils_1.makeRawRequest(downloadVideos[0].files[0].fileDownloadUrl, 200);
                    yield extra_utils_1.makeRawRequest(downloadVideos[2].files[0].fileDownloadUrl, 200);
                }
                {
                    const res = yield extra_utils_1.makeRawRequest(downloadVideos[2].streamingPlaylists[0].files[0].fileDownloadUrl, 403);
                    expect(res.body.error).to.equal('Sun Jian');
                    yield extra_utils_1.makeRawRequest(downloadVideos[2].files[0].fileDownloadUrl, 200);
                    yield extra_utils_1.makeRawRequest(downloadVideos[0].streamingPlaylists[0].files[0].fileDownloadUrl, 200);
                    yield extra_utils_1.makeRawRequest(downloadVideos[1].streamingPlaylists[0].files[0].fileDownloadUrl, 200);
                }
            });
        });
    });
    describe('Embed filters', function () {
        ...
        it('Should run filter:html.embed.video.allowed.result', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                const res = yield extra_utils_1.makeRawRequest(servers[0].url + embedVideos[0].embedPath, 200);
                expect(res.text).to.equal('Lu Bu');
            });
        });
        it('Should run filter:html.embed.video-playlist.allowed.result', function () {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                const res = yield extra_utils_1.makeRawRequest(servers[0].url + embedPlaylists[0].embedPath, 200);
