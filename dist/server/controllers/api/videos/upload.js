...
const uploadxMiddleware = core_1.uploadx.upload({ directory: upload_1.getResumableUploadPath() });
...
const reqVideoFileAddResumable = express_utils_1.createReqFiles(['thumbnailfile', 'previewfile'], constants_1.MIMETYPES.IMAGE.MIMETYPE_EXT, {
    thumbnailfile: upload_1.getResumableUploadPath(),
    previewfile: upload_1.getResumableUploadPath()
});
uploadRouter.post('/upload', doc_1.openapiOperationDoc({ operationId: 'uploadLegacy' }), middlewares_1.authenticate, reqVideoFileAdd, middlewares_1.asyncMiddleware(middlewares_1.videosAddLegacyValidator), middlewares_1.asyncRetryTransactionMiddleware(addVideoLegacy));
uploadRouter.post('/upload-resumable', doc_1.openapiOperationDoc({ operationId: 'uploadResumableInit' }), middlewares_1.authenticate, reqVideoFileAddResumable, middlewares_1.asyncMiddleware(middlewares_1.videosAddResumableInitValidator), uploadxMiddleware);
uploadRouter.delete('/upload-resumable', middlewares_1.authenticate, uploadxMiddleware);
uploadRouter.put('/upload-resumable', doc_1.openapiOperationDoc({ operationId: 'uploadResumable' }), middlewares_1.authenticate, uploadxMiddleware, middlewares_1.asyncMiddleware(middlewares_1.videosAddResumableValidator), middlewares_1.asyncMiddleware(addVideoResumable));
...
function addVideo(options) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const videoResolutionInfo = yield ffprobe_utils_1.getVideoFileResolution(videoPhysicalFile.path);
        ...
        const videoData = video_1.buildLocalVideoFromReq(videoInfo, videoChannel.id);
        videoData.state = video_state_1.buildNextVideoState();
        ...
        videoPhysicalFile.filename = path_1.basename(destination);
        videoPhysicalFile.path = destination;
        const [thumbnailModel, previewModel] = yield video_1.buildVideoThumbnailsFromReq({
            video,
            files,
            fallback: type => thumbnail_1.generateVideoMiniature({ video, videoFile, type, size }),
            size
        });
        ...
        createTorrentFederate(video, videoFile)
            .then(() => {
            if (video.state === 6) {
                return video_1.addMoveToObjectStorageJob(video);
            }
            if (video.state === 2) {
                return video_1.addOptimizeOrMergeAudioJob(videoCreated, videoFile, user);
            }
        })
        ...
        return res.json({
            video: {
                id: videoCreated.id,
                shortUUID: uuid_1.uuidToShort(videoCreated.uuid),
...
function buildNewFile(videoPhysicalFile) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const videoFile = new video_file_1.VideoFileModel({
            extname: core_utils_1.getLowercaseExtension(videoPhysicalFile.filename),
        ...
        if (videoFile.isAudio()) {
            ...
            videoFile.resolution = (yield ffprobe_utils_1.getVideoFileResolution(videoPhysicalFile.path)).resolution;
        }
        videoFile.filename = paths_1.generateWebTorrentVideoFilename(videoFile.resolution, videoFile.extname);
        return videoFile;
    });
}
