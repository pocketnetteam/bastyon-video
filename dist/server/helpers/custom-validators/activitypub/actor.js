...
function isActorEndpointsObjectValid(endpointObject) {
    if (endpointObject === null || endpointObject === void 0 ? void 0 : endpointObject.sharedInbox) {
        return (0, misc_2.isActivityPubUrlValid)(endpointObject.sharedInbox);
    }
    return true;
}
exports.isActorEndpointsObjectValid = isActorEndpointsObjectValid;
function isActorPublicKeyObjectValid(publicKeyObject) {
    return (0, misc_2.isActivityPubUrlValid)(publicKeyObject.id) &&
        (0, misc_2.isActivityPubUrlValid)(publicKeyObject.owner) &&
...
function isActorPublicKeyValid(publicKey) {
    return (0, misc_1.exists)(publicKey) &&
...
function isActorPreferredUsernameValid(preferredUsername) {
    return (0, misc_1.exists)(preferredUsername) && validator_1.default.matches(preferredUsername, actorNameRegExp);
}
...
function isActorFollowingCountValid(value) {
    return (0, misc_1.exists)(value) && validator_1.default.isInt('' + value, { min: 0 });
}
...
function sanitizeAndCheckActorObject(actor) {
    ...
    return (0, misc_1.exists)(actor) &&
        (0, misc_2.isActivityPubUrlValid)(actor.id) &&
        isActorTypeValid(actor.type) &&
        (0, misc_2.isActivityPubUrlValid)(actor.inbox) &&
        isActorPreferredUsernameValid(actor.preferredUsername) &&
        (0, misc_2.isActivityPubUrlValid)(actor.url) &&
        isActorPublicKeyObjectValid(actor.publicKey) &&
        isActorEndpointsObjectValid(actor.endpoints) &&
        (!actor.outbox || (0, misc_2.isActivityPubUrlValid)(actor.outbox)) &&
        (!actor.following || (0, misc_2.isActivityPubUrlValid)(actor.following)) &&
        (!actor.followers || (0, misc_2.isActivityPubUrlValid)(actor.followers)) &&
        (0, misc_2.setValidAttributedTo)(actor) &&
...
function normalizeActor(actor) {
    ...
    if (!(0, misc_1.isDateValid)(actor.published))
        actor.published = undefined;
    if (actor.summary && typeof actor.summary === 'string') {
        actor.summary = (0, core_utils_1.peertubeTruncate)(actor.summary, { length: constants_1.CONSTRAINTS_FIELDS.USERS.DESCRIPTION.max });
...
function isValidActorHandle(handle) {
    if (!(0, misc_1.exists)(handle))
        return false;
    const parts = handle.split('@');
    if (parts.length !== 2)
        return false;
    return (0, servers_1.isHostValid)(parts[1]);
}
exports.isValidActorHandle = isValidActorHandle;
function areValidActorHandles(handles) {
    return (0, misc_1.isArray)(handles) && handles.every(h => isValidActorHandle(h));
}
